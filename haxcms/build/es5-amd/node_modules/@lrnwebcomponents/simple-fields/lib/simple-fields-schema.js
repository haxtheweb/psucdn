define(["exports", "require", "../../../lit-element/lit-element.js", "./simple-fields-array.js"], function (_exports, _require, _litElement, _simpleFieldsArray) {
  ...
  function _templateObject2_0d308bc0598111eabae509a8b8475ead() {
    var data = babelHelpers.taggedTemplateLiteral([""]);

    _templateObject2_0d308bc0598111eabae509a8b8475ead = function _templateObject2_0d308bc0598111eabae509a8b8475ead() {
      return data;
    };
  ...
  function _templateObject_0d308bc0598111eabae509a8b8475ead() {
    var data = babelHelpers.taggedTemplateLiteral(["\n      <div id=\"schema-fields\" aria-live=\"polite\">\n        <slot></slot>\n      </div>\n    "]);

    _templateObject_0d308bc0598111eabae509a8b8475ead = function _templateObject_0d308bc0598111eabae509a8b8475ead() {
      return data;
    };
  ...
  function (_LitElement) {
    ...
    babelHelpers.createClass(SimpleFieldsSchema, [{
      ...
      value: function render() {
        return (0, _litElement.html)(_templateObject_0d308bc0598111eabae509a8b8475ead());
      }
    }], [{
      key: "styles",
      get: function get() {
        return [(0, _litElement.css)(_templateObject2_0d308bc0598111eabae509a8b8475ead())];
      }
    }]);
    ...
    function SimpleFieldsSchema() {
      ...
      setTimeout(function () {
        new Promise(function (res, rej) {
          return _require.default(["./simple-fields-fieldset.js"], res, rej);
        });
        new Promise(function (res, rej) {
    ...
    babelHelpers.createClass(SimpleFieldsSchema, [{
       ...
       */
      value: function _buildFormElement(config) {
        var _this2 = this;

        var parent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this;
        var index = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -1;
        var slot = arguments.length > 3 ? arguments[3] : undefined;

        var el = document.createElement(config.component.type.element),
            elname = index > -1 ? config.name.replace("..", ".".concat(index, ".")) : config.name,
        ...
        Object.keys(config.component.type).forEach(function (key) {
        ...
        if (config.component.type.isArray && index < 0) {
          el.addEventListener("add", function (e) {
            return _this2._setValue("".concat(elname, ".").concat(elval.length), {});
          });
          el.addEventListener("remove", function (e) {
            var temp = _this2._deepClone(elval);

            temp.splice(parseInt(e.detail.id.replace(/item-/, "")), 1);
            console.log(temp, parseInt(e.detail.id.replace(/item-/, "")));

            _this2._setValue("".concat(elname), temp);
          });
          if (config.sortBy) elval = elval.sort(function (a, b) {
            var i = 0,
                ai = 0,
                bi = 0;
          ...
          if (elval) elval.forEach(function (item, i) {
            ...
            var id = "item-".concat(i),
                ...
                previewKeys = keys.filter(function (key) {
              return config.schema.properties[key].previewField === true;
            });
            if (previewKeys.length < 1) previewKeys.push(keys[0]);
            /* adds fields to array items */

            keys.forEach(function (key) {
              var childname = config.schema.properties[key].name.replace("".concat(elname, ".."), "");

              _this2._buildFormElement(config.schema.properties[key], child, i, sortSlot && config.sortBy[0] === childname ? sortSlot : previewKeys.includes(key) ? previewSlot : undefined);
            });
          });
        } else if (config.schema && config.schema.properties) {
          /* gets nested fields for a fieldset */
          Object.keys(config.schema.properties).forEach(function (key) {
            return _this2._buildFormElement(config.schema.properties[key], el);
          });
        } else {
          el[config.component.valueProperty] = elval;

          el.onchange = function (e) {
            return _this2._setValue(elname, el[config.component.valueProperty]);
          };
        }
      }
      /**
      ...
      value: function _getProperties() {
        ...
        return Object.keys(target.properties || []).map(function (key) {
          ...
          Object.keys(root.dataTypes).forEach(function (key) {
            if (Array.isArray(schema.type) && schema.type.indexOf(key) !== -1 || schema.type === key) {
              property.component.type = _this3._deepClone(root.dataTypes[key]);
              ...
              if (property.component.type.isFieldset || property.component.type.isArray) {
                ...
                if (schema.items && schema.items.properties) {
                  ...
                  property.schema.properties = _this3._getProperties(schema.items, property.component.type.isArray ? "".concat(property.name, ".") : property.name);
                }
              }
            }
          });
      ...
      value: function updated(changedProperties) {
        ...
        changedProperties.forEach(function (oldValue, propName) {
          ...
          if (propName === "value") _this4._valueChanged(_this4.value, oldValue);
        });
      }
      /**
       * clears the form
      ...
      value: function _rebuildForm() {
        ...
        if (this.schema) {
          var formProperties = this._getProperties(this.schema);

          formProperties.forEach(function (property) {
            return _this5._buildFormElement(property);
          });
        }
      }
      /**
